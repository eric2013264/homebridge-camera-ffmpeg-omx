...
var globalUploader = false;
var globalDrive = new drive();
var name = "";
...
function FFMPEG(hap, cameraConfig, videoProcessor) {
  ...
  globalUploader = cameraConfig.uploader || false;

  if (globalUploader)
  {
     //this.drive = new drive();
     globalDrive = new drive();
  }
...
//  let resolution = request.width + 'x' + request.height;
  ...
  ffmpeg.on('close', function(code) {
    ...
    if (globalUploader)
    {
      globalDrive.storePicture(name, imageBuffer);
      console.log("Uploaded snapshot: " + resolution + " to Google Drive");
    }
...
FFMPEG.prototype.handleStreamRequest = function(request) {
  ...
  if (sessionID) {
    ...
    if (requestType == "start") {
      ...
      if (sessionInfo) {
        var width = globalMaxWidth;
        var height = globalMaxHeight;
        var fps = 30;
        var bitrate = 300;
	...
	let ffmpegCommand = this.ffmpegSource + ' -threads 0 -vcodec ' + vcodec + ' -an -pix_fmt yuv420p -r '+ fps +' -f rawvideo -tune zerolatency -vf scale='+ width +':'+ height +' -b:v '+ bitrate +'k -bufsize '+ bitrate +'k -payload_type 99 -ssrc ' + videoSsrc + ' -f rtp -srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params '+videoKey.toString('base64')+' srtp://'+targetAddress+':'+targetVideoPort+'?rtcpport='+targetVideoPort+'&localrtcpport='+targetVideoPort+'&pkt_size=' + packetsize;
        console.log("Started streaming video from " + name + ": " + width + "x" + height + "@" + bitrate + "kBit|" + fps + "fps|packet size:" + packetsize + " to " + sessionInfo["address"]);
